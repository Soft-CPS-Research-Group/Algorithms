...
class BehaviorCloningRegularizer:
    """Store immutable teacher demonstrations and calculate BC-only actor losses.

    The only supported teacher is ``RBCSmartPolicy``.  Demonstrations must use
    the full encoded representation for their stored ``BuildingTokenLayout``;
    they are grouped by that layout's signature and cannot be reused across an
    incompatible topology.  A checkpoint persists demonstration data and
    sampler/metric state, but never a live teacher.  The caller must therefore
    reattach the environment before resuming, and restored arrays are copied
    back into the same read-only representation used at collection time.
    """
    ...
    def record_demonstration(
        ...
        if copied_observation.shape != expected_observation_shape:
            ...
            return
        if not np.isfinite(copied_observation).all():
            self._rejected_at_record += 1
            logger.warning(
                "event=bc_demonstration_rejected building_idx={} "
                "reason=observation_nonfinite expected_shape={} actual_shape={} "
                "expected_length={} actual_length={}",
                building_idx,
                expected_observation_shape,
                copied_observation.shape,
    ...
    def demonstration_loss(
        ...
        *,
        ...
        normalizer = weights.expand_as(error).sum()
        raw_loss = error.sum() / normalizer.clamp_min(torch.finfo(error.dtype).eps)
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        return {
            "demonstrations": self._freeze_demonstrations(self._demonstrations),
    ...
    def load_state_dict(self, state: Mapping[str, Any]) -> None:
        ...
        self._demonstrations = self._freeze_demonstrations(state["demonstrations"])
    ...
    @staticmethod
    def _freeze_demonstrations(
        demonstrations_by_building: Mapping[int, List[Demonstration]],
    ) -> Dict[int, List[Demonstration]]:
        """Copy persisted demonstrations back into the immutable live contract."""
        frozen: Dict[int, List[Demonstration]] = {}
        for building_idx, demonstrations in demonstrations_by_building.items():
            frozen[int(building_idx)] = []
            for demonstration in demonstrations:
                observation = np.asarray(demonstration.observation, dtype=np.float32).copy()
                target = np.asarray(demonstration.target, dtype=np.float32).copy()
                observation.setflags(write=False)
                target.setflags(write=False)
                frozen[int(building_idx)].append(
                    Demonstration(
                        observation=observation,
                        encoded_length=int(demonstration.encoded_length),
                        layout=deepcopy(demonstration.layout),
                        layout_signature=tuple(demonstration.layout_signature),
                        target=target,
                    )
                )
        return frozen
