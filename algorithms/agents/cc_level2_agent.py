...
class CCLevel2Agent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        hidden_dims = hyper.get("hidden_dims", [256, 256])
        self.policy = CommunityMarketMakerNetV2(
            self._c_dim, self._num_buildings, hidden_dims
        )
    ...
    def _rebuild_for_num_buildings(self) -> None:
        ...
        hidden_dims = list(self.policy.encoder[0].in_features
                           for layer in self.policy.encoder
                           if isinstance(layer, nn.Linear))
        # Re-derive hidden_dims from existing policy if possible.
        hd: List[int] = []
        for layer in self.policy.encoder:
            if isinstance(layer, nn.Linear):
                hd.append(layer.out_features)
        if not hd:
            hd = [256, 256]
        self.policy = CommunityMarketMakerNetV2(self._c_dim, self._num_buildings, hd)
        self.ppo_optim = Adam(self.policy.parameters(), lr=1e-4)
        self.rollout_buffer = RolloutBufferV2(
    ...
    def _log_decision(self) -> None:
        ...
        for i in range(self._num_buildings):
            ...
            if base + 5 < len(ctx):
                ...
                actual_hours = (1.0 - urgency_24h) * 24.0
                urgency = max(1.0 - actual_hours / 4.0, 0.0)  # hardcoded 4h matches CCRewardLevel2 default
