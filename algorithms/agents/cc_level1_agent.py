...
class CCLevel1Agent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # "actions": execute battery actions directly (standalone mode).
        # "signal":  return float (-1/0/+1) as Pipeline context for downstream RBC.
        self._output_mode: str = hyper.get("output_mode", "actions")
        self._step_in_interval   = 0

        # Decision-quality shaping weight.
        # Combats neutral-collapse: adds immediate gradient -signal × price_delta
        # on top of the noisy community-cost env reward.
        # Set to 0.0 to use env reward only.
        self._shaping_weight: float = float(hyper.get("shaping_weight", 1.0))
    ...
    def predict(
        ...
        *,
        ...
        if self._output_mode == "signal":
            # Return scalar signal as Pipeline context for downstream RBC.
    ...
    def update(
        ...
        *,
        ...
        # CC reward = community outcome (env) + decision-quality shaping (internal).
        # Shaping term: -signal × price_delta fires immediately on each decision,
        # providing a clean gradient that cuts through baseline-load noise.
    ...
    def export_artifacts(self, output_dir, context=None):
        ...
        return {
            "format": "onnx",
            "artifacts": [{"path": str(export_path.relative_to(export_root)), "format": "onnx"}],
        }
    ...
    def _compute_signal_quality(self) -> float:
        """
        Immediate decision-quality shaping: -signal_value × price_delta.

        signal_value ∈ {-1, 0, +1}
        price_delta  = price_now − mean(price_now, pred_1, pred_2, pred_3)

        Behaviour:
            reduce   (-1) when price > ref  → +reward  (cut when expensive) ✓
            increase (+1) when price < ref  → +reward  (load up when cheap)  ✓
            neutral  ( 0)                   →  0        (no bonus, no penalty)
            reduce   (-1) when price < ref  → -reward  (wrong direction)     ✓
            increase (+1) when price > ref  → -reward  (wrong direction)     ✓

        Returns 0.0 if no decision has been cached yet.
        This is computed every env step, accumulated over the interval.
        """
        ...
        signal      = float(_ACTION_TO_BATTERY[self._cached_action])   # -1, 0, +1
