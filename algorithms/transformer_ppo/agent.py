...
@dataclass
class _TrainingRuntimeSnapshot:
    """Mutable training state required for transactional rollback."""
...
class AgentTransformerPPO(BaseAgent):
        ...
        *,
        ...
        try:
            ...
            if not update_step:
                return
            training_snapshot = (
                self._snapshot_training_runtime_state()
            ...
            if "training_snapshot" in locals() and training_snapshot is not None:
                self._restore_training_runtime_state(training_snapshot)
            else:
    ...
    def _snapshot_training_runtime_state(self) -> _TrainingRuntimeSnapshot:
        states = self._per_building
        return _TrainingRuntimeSnapshot(
    ...
    def _restore_training_runtime_state(
        self, snapshot: _TrainingRuntimeSnapshot
    ) -> None:
        for index, state in enumerate(self._per_building):
        ...
        self._current_episode = snapshot.current_episode
        self._bc_pretraining_complete = snapshot.bc_pretraining_complete
        self._bc_actor_training_step = snapshot.bc_actor_training_step
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        runtime_snapshot = self._snapshot_training_runtime_state()
        try:
            ...
            self._restore_training_runtime_state(runtime_snapshot)
            raise
