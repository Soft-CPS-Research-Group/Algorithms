"""KPI-aligned reward for offline RL.
...
``scripts/calibrate_reward.py``) and frozen into
``datasets/offline_rl/derived/reward_weights.json``. The same weights
...
depend on the env, only on the columns in ``schema``.
"""
...
"""Initial-guess weights from ``docs/offline_rl/reward_design.md`` §4.2.
...
class RewardState:
    """Mutable state carried across calls to :func:`compute_reward`.
