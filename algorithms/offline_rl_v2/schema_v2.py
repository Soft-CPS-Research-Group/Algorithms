"""Schema v2 — single source of truth for the offline-RL dataset.

Every v2 component (collector, loader, trainer, reward calibrator) imports
...
with BC-v2 / IQL-v2 / etc., only the **values** in ``ACTION_COLUMNS``,
...
def build_arrow_schema() -> pa.Schema:
    ...
    fields.append(pa.field("reward_v2", _FLOAT))
...
class SchemaError(RuntimeError):
    """Raised when collected data doesn't match the v2 schema."""
...
def validate_observation_names(env_observation_names: List[str]) -> None:
    ...
    if actual != expected:
        ...
        raise SchemaError(
            "Building 5 observation names do not match v2 schema.\n"
