"""BC-v2 inference agent.

CityLearn ``BaseAgent`` that:

  * Controls **Building 5** with the trained BC-v2 policy.
  * Controls the other 16 buildings with ``RuleBasedPolicyV2`` — the same
...
Loading
-------
    agent = BCAgentV2.from_seed_dir(
        Path("runs/offline_bc_v2/<run>/seed_<N>"),
        rbc_config=None,  # default-empty hyperparameters
    )

The seed dir must contain ``policy.pt``, ``obs_standardiser.npz``, and
``architecture.json`` (all written by ``bc_trainer_v2.train_single_seed``).
...
from algorithms.offline_rl_v2.bc_dataset_v2 import ObservationStandardiser
...
class BCAgentV2(BaseAgent):
    """BC for Building 5, RBC-v2 for the other buildings."""
    ...
    def predict(  # type: ignore[override]
        ...
        if not self._attached:
            raise RuntimeError(
                "BCAgentV2.predict called before attach_environment"
            )
