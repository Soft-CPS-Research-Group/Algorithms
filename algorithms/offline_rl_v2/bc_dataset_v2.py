"""BC-v2 dataset utilities.

Reads ``datasets/offline_rl_v2/derived/rbc_with_reward_v2.parquet`` (or any
v2-schema-compatible parquet), extracts the 35-D obs and 2-D action vectors
...
def load_obs_action_arrays(
    ...
    """Load (obs, action) arrays from a v2-schema parquet.

    Returns
    -------
    obs : (N, 35) float32
    action : (N, 2) float32
    obs_feature_names : list of 35 names (from schema_v2.OBSERVATION_NAMES)
    action_feature_names : list of 2 names (from schema_v2.ACTION_NAMES)
    """
...
class BCDatasetV2(Dataset):
    ...
    training; the v2 dataset (~88k rows × 70 floats × 4 bytes ≈ 25 MB) easily
    fits.
    """
...
def make_split(
    ...
    *,
    ...
    return BCDatasetSplit(
        train=BCDatasetV2(train_obs_std, train_act),
        val=BCDatasetV2(val_obs_std, val_act),
