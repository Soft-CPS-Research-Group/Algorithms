"""Calibrate the reward weights against RBC rollouts.

Procedure (matches docs/offline_rl/reward_design.md §4.1):

 1. Load every ``seed_*.parquet`` in ``datasets/offline_rl/rbc/`` plus
 ...
 5. Sanity check: Spearman ρ between ``-Σ_t reward_t`` (per-rollout)
 and ``y^k`` must be ≥ 0.9.
 6. Round weights to 2 significant figures, freeze JSON.

Fallback: if NNLS is degenerate (rank < 4, or all weights ≈ 0, or
Spearman ρ < 0.9), use ``reward.DEFAULT_WEIGHTS`` and log the reason.

Outputs (under ``datasets/offline_rl/derived/``):
 - ``reward_weights.json`` — frozen weights + provenance.
 - ``rbc_with_reward.parquet`` — RBC dataset with reward column.
 - ``reward_breakdown.parquet`` — per-step term breakdown.
 - ``reward_calibration.log`` — full diagnostics, human-readable.

Usage
-----
 .venv/bin/python -m scripts.calibrate_reward
 .venv/bin/python -m scripts.calibrate_reward --rbc-dir <path> --out-dir <path>
"""
...
DEFAULT_RBC_DIR = REPO_ROOT / "datasets" / "offline_rl" / "rbc"
DEFAULT_OUT_DIR = REPO_ROOT / "datasets" / "offline_rl" / "derived"
 ...
 *,
 ...
 # 7. Sanity check — Spearman between -Σ reward and KPI target.
 per_seed_neg_reward = []
 for df in frames:
 ...
 logger.info(
 "Spearman ρ(-Σreward, KPI_target) = %.4f (p=%.4f, threshold=%.2f)",
 rho_f, pval_f, SPEARMAN_THRESHOLD,
 )
 diag["spearman_rho"] = rho_f
 diag["spearman_pvalue"] = pval_f
 diag["per_seed_neg_reward_sum"] = dict(zip(seeds, [float(x) for x in per_seed_neg_reward]))
...
def _write_per_step_outputs(
 ...
 aug_path = out_dir / "rbc_with_reward.parquet"
 bk_path = out_dir / "reward_breakdown.parquet"
...
def _build_logger(log_path: Path) -> logging.Logger:
 ...
 logger = logging.getLogger("calibrate_reward")
...
def main() -> int:
 parser = argparse.ArgumentParser(description="Calibrate reward weights against RBC rollouts.")
 parser.add_argument("--rbc-dir", type=Path, default=DEFAULT_RBC_DIR)
 parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
 args = parser.parse_args()

 log_path = args.out_dir / "reward_calibration.log"
 logger = _build_logger(log_path)

 logger.info("=== reward calibration ===")
 ...
 "nnls_residual",
 "weight_source",
 "per_seed_neg_reward_sum",
 )
 if k in diag
 },
 }
 weights_path = args.out_dir / "reward_weights.json"
