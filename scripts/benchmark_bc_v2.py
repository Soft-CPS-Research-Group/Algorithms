"""Benchmark BC-v2 vs RBC-v2 on Building 5.

Self-contained v2 benchmark. Mirrors :mod:`scripts.benchmark_bc_m3` in
structure but:

* Uses **v2 RBC** (``RuleBasedPolicyV2``) as the baseline — the same
  controller that produced the offline dataset, so BC's parity is measured
  against its own behaviour policy.
* Loads BC-v2 checkpoints via ``BCAgentV2.from_seed_dir``.
...
* No Random comparator — the v1 benchmark already established Random ≫ RBC
  on this task; re-running it adds noise without information for v2.

Multi-seed schema
-----------------
* **RBC-v2** : ``len(env_seeds)`` rollouts, one per env seed.
* **BC-v2**  : ``len(bc_seeds) × len(env_seeds)`` rollouts. For each BC
...
.. code-block:: bash
    ...
    .venv/bin/python -m scripts.benchmark_bc_v2 \\
        --bc-root runs/offline_bc_v2/run-001 \\
        --output  docs/offline_rl_v2/bc_v2_vs_rbc_v2_benchmark.md \\
        --smoke
...
from algorithms.offline_rl_v2.bc_agent_v2 import BCAgentV2  # noqa: E402
from algorithms.offline_rl_v2.rbc_v2 import RuleBasedPolicyV2  # noqa: E402
...
DEFAULT_EVAL_SEEDS_V2: Tuple[int, ...] = tuple(range(200, 210))


# ---------------------------------------------------------------------------
# v2 controller rollouts
# ---------------------------------------------------------------------------


def _make_rbc_v2() -> RuleBasedPolicyV2:
    """RBC-v2 with the standard schema config."""
    return RuleBasedPolicyV2(
        config={
...
def _run_rbc_v2_rollouts(env_seeds: Sequence[int]) -> List[Dict[str, Any]]:
    runs: List[Dict[str, Any]] = []
    for env_seed in env_seeds:
        agent = _make_rbc_v2()
        runs.append(rollout(agent, env_seed=env_seed, label="RBC-v2"))
    return runs


def _discover_bc_v2_seed_dirs(root: Path) -> List[Tuple[int, Path]]:
    """Return ``(seed, seed_dir)`` for every ``seed_<N>/`` under ``root`` with
    a ``policy.pt`` and ``architecture.json`` (BC-v2 layout).
    """
    if not root.is_dir():
        raise FileNotFoundError(f"BC-v2 root not a directory: {root}")
    ...
    if not found:
        raise FileNotFoundError(
            f"No BC-v2 seed dirs (policy.pt + architecture.json) under {root}"
        )
    return found


def _run_bc_v2_rollouts(
    ...
    for train_seed, seed_dir in bc_seeds:
        for env_seed in env_seeds:
            agent = BCAgentV2.from_seed_dir(seed_dir)
            run = rollout(agent, env_seed=env_seed, label=f"BC-v2-s{train_seed}")
...
# ---------------------------------------------------------------------------
# Report (2-column: RBC-v2 vs BC-v2)
# ---------------------------------------------------------------------------
...
def _render_kpi_table(
    ...
    *,
    ...
    lines = [
        f"### {title}",
        "",
        "| KPI | RBC-v2 (mean ± std) | BC-v2 (mean ± std) | "
        "Δ (BC − RBC) | Verdict |",
        "|---|---:|---:|---:|:---|",
    ]
    for k in keys:
        ...
        delta, verdict = delta_significance(
            ca, ba, cand_label="BC-v2", base_label="RBC-v2"
        )
        lines.append(
...
def _build_report(
    *,
    ...
    return f"""# BC-v2 vs RBC-v2 — CityLearn Benchmark

_Generated_: {timestamp}{smoke_note}
_Dataset_: `{DATASET_SCHEMA}` (interface=`flat`, topology_mode=`static`)
_Reward function_: `V2GPenaltyReward` (env reward; v2 reward is a separate scalar used for IQL)
_Target building_: **{TARGET_BUILDING_NAME}** (agent index {TARGET_BUILDING_INDEX})
_BC-v2 checkpoint root_: `{bc_root}`
_BC-v2 training seeds_: [{train_seed_list}]
_Env seeds_ (per controller): [{env_seed_list}]  ← disjoint from RBC dataset seeds 22..31
_Total rollouts_: RBC-v2={len(rbc_runs)}, BC-v2={len(bc_runs)}
...
>
> The BC-v2 agent controls Building 5 only; the other 16 agents are driven by
> a fresh RBC-v2 instance (so off-target buildings are bit-identical between
...
## 5. Success criterion (v2)

> BC-v2 succeeds if its district headline KPIs are **within 1σ of RBC-v2** on
> all of cost / peak / ramping / unserved-energy. That demonstrates the v2
> pipeline reproduces its behaviour policy faithfully — a precondition for
> the IQL-v2 step that follows.
>
> A *strict improvement* over RBC is not expected from BC alone (BC is
> imitation, not optimisation); that's IQL-v2's job in the next step.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Root dir produced by scripts/train_bc_v2.py (contains seed_<N>/).",
    )
    parser.add_argument(
        "--output",
        default="docs/offline_rl_v2/bc_v2_vs_rbc_v2_benchmark.md",
        help="Output markdown report path (relative to repo root).",
    )
    parser.add_argument(
        "--env-seeds", default=",".join(str(s) for s in DEFAULT_EVAL_SEEDS_V2),
...
def main() -> int:
    ...
    bc_seeds = _discover_bc_v2_seed_dirs(bc_root)
    if args.smoke:
        bc_seeds = bc_seeds[:1]
        env_seeds = env_seeds[:1]
        print(f"[smoke] using bc_seeds={[s for s,_ in bc_seeds]}, env_seeds={env_seeds}")

    print(f"[plan] BC-v2 training seeds: {[s for s,_ in bc_seeds]}")
    print(f"[plan] env seeds            : {env_seeds}")
    total_bc = len(bc_seeds) * len(env_seeds)
    print(f"[plan] total rollouts       : RBC-v2={len(env_seeds)}, BC-v2={total_bc}")

    print("[run] RBC-v2...")
    rbc_runs = _run_rbc_v2_rollouts(env_seeds)
    print(f"[run] RBC-v2 done ({len(rbc_runs)} rollouts)")

    print("[run] BC-v2...")
    bc_runs = _run_bc_v2_rollouts(bc_seeds, env_seeds)
    print(f"[run] BC-v2 done ({len(bc_runs)} rollouts)")
    ...
    raw_dir = output_path.parent / "bc_v2_vs_rbc_v2_raw"
    ...
    payload = {
        ...
        "aggregates": {
            "district": {
                "RBC-v2": _full_agg(rbc_runs, "district"),
                "BC-v2": _full_agg(bc_runs, "district"),
            },
            "building_5": {
                "RBC-v2": _full_agg(rbc_runs, "building"),
                "BC-v2": _full_agg(bc_runs, "building"),
            },
        },
        "per_rollout": {
            "RBC-v2": [
    ...
    for label, runs in (("rbc_v2", rbc_runs), ("bc_v2", bc_runs)):
        for r in runs:
