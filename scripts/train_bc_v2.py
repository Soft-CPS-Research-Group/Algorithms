"""Train BC-v2 on the RBC v2 dataset.

Multi-seed driver around ``algorithms.offline_rl_v2.bc_trainer_v2``.

Inputs
------
* ``--dataset`` — Parquet with v2 schema. Defaults to
...
Smoke-run (1 seed, 5 epochs, tiny net)::
    ...
    .venv/bin/python -m scripts.train_bc_v2 \\
        --output runs/offline_bc_v2/smoke \\
        --seeds 100 \\
        --epochs 5 \\
        --hidden-layers 64,64
...
Full run (5 seeds, 50 epochs, default arch)::
    ...
    .venv/bin/python -m scripts.train_bc_v2 \\
        --output runs/offline_bc_v2/run-001
"""
...
from algorithms.offline_rl_v2.bc_trainer_v2 import (  # noqa: E402
    BCTrainingConfigV2,
    train_multi_seed,
)
...
def main() -> int:
    ...
    print(f"[train_bc_v2] dataset={args.dataset}")
    print(f"[train_bc_v2] output={args.output}")
    print(f"[train_bc_v2] seeds={seeds}")
    print(f"[train_bc_v2] config={config}")
    ...
    print("\n=== BC-v2 multi-seed training complete ===")
