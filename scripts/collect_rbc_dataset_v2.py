"""Collect an offline-RL v2 dataset by rolling out the RuleBasedPolicy.

Output (per seed):
    datasets/offline_rl_v2/rbc/seed_<N>.parquet
...
* `reward_env` is the env-returned reward for Building 5 (V2GPenaltyReward
  per-building component, before community mixing). `reward_v2` is filled
  with NaN here and computed later by the reward module.
* Collector **fails fast** on:
    - schema mismatch (env obs/action names ≠ schema_v2 expectations);
    - zero action variance in any action column (the M2 failure mode).

Usage
-----
    .venv/bin/python -m scripts.collect_rbc_dataset_v2 --seeds 22
    .venv/bin/python -m scripts.collect_rbc_dataset_v2 --seeds 22 23 24 25 26 \\
...
from algorithms.offline_rl_v2 import schema_v2 as S  # noqa: E402
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "datasets" / "offline_rl_v2" / "rbc"
...
# RBC v2 controls only the EV charger; the stationary battery action is left
# at 0. Other behaviour policies (BC v2, IQL v2, ...) are expected to use both.
RBC_V2_EXPECTED_CONSTANT_ACTIONS: tuple[str, ...] = (
...
def _build_rbc(env_metadata: Dict[str, Any]) -> RuleBasedPolicyV2:
...
def collect_seed(seed: int) -> Dict[str, Any]:
    ...
    # Validate B5 obs/action names against the v2 schema before doing work.
    ...
    while not (terminated or truncated):
        ...
        row["reward_v2"] = float("nan")  # filled by reward module later
...
def _rows_to_table(rows: Sequence[Dict[str, Any]]) -> pa.Table:
    """Convert rows to an Arrow Table that strictly matches the v2 schema."""
...
def assert_action_variance(
    ...
    *,
    ...
    Some action columns are *legitimately* constant — e.g. RBC v2 doesn't
    ...
    if warned:
        print(
            "[collect_v2] note: zero-variance action columns (expected for this "
            "behaviour policy):\n"
...
def write_manifest(
    ...
    *,
    ...
    manifest = {
        "behaviour_policy": "rbc_v2",
        "behaviour_policy_class": "algorithms.offline_rl_v2.rbc_v2.RuleBasedPolicyV2",
...
def main(argv: List[str] | None = None) -> int:
    ...
    print(f"[collect_v2] writing to: {out_dir}")
    print(f"[collect_v2] seeds: {seeds}")
    print(f"[collect_v2] schema_hash: {S.schema_hash()[:16]}…")
    ...
    for seed in seeds:
        ...
        if out_path.exists() and not args.smoke:
            print(f"[collect_v2] skipping existing {out_path.name}")
        ...
        assert_action_variance(astats, expected_zero_variance=RBC_V2_EXPECTED_CONSTANT_ACTIONS)
        ...
        if first_table is None:
            first_table = table
        print(
            f"[collect_v2] seed={seed} rows={table.num_rows} "
    ...
    if not runs:
        ...
        if not kpi_summary_path.exists():
            print("[collect_v2] no fresh runs and kpi_summary.csv missing — re-running rollouts")
    ...
    print(f"\n[collect_v2] manifest: {manifest_path}")
    print(f"[collect_v2] total transitions: {n_steps_total}")
    print(f"[collect_v2] action stats (per-seed mean of std):")
    for col in S.ACTION_COLUMNS:
        print(f"    {col}: std≈{agg_action[col]['mean_of_stds']:.4f}  "
              f"range=[{agg_action[col]['min_overall']:.3f}, {agg_action[col]['max_overall']:.3f}]")
    print(f"[collect_v2] reward_env: mean≈{agg_reward['mean_of_means']:.3f}, "
