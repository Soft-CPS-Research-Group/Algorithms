...
### 5.9 The silent step cap (benchmark), Bug 9

Phase 13 was scoped to re-run the benchmark stage from §7 at full-year rollout horizons (35,040 steps × 10 eval seeds, one full simulated year of 15-minute control per seed) to close out the 1-day-rollout caveat that the original benchmark carried. The re-run launched with the orchestrator forwarding `--episode-steps 35040` end-to-end (the Phase A commit `2931aba` that extended the same forwarding logic already used for the collector to the benchmark subprocess). It completed cleanly and produced a `results.json` that looked correct at first glance — thirty entries, ten seeds per algorithm, all KPIs populated. Only under closer inspection did the numbers become suspect: IQL and CQL had *worsened* against the 1-day baseline that Phase 13 was meant to supersede, which no plausible offline-RL evaluation should show as the horizon lengthens. A one-line jq check on each entry's `steps` field revealed the reason: `RBCSmart` entries reported `steps: 35039`, but every `IQL` and `CQL` entry reported `steps: 6000`. The three algorithms had run different horizons in the same `results.json`.

**Root cause.** [`scripts/benchmark_entity_agents.py`](../../scripts/benchmark_entity_agents.py):`entity_rollout()` had a hardcoded `max_steps: int = 6000` default and, at the bottom of its stepping loop, an unconditional `if step >= max_steps: break`. The companion `rbc_rollout()` had no such cap — it ran until the environment set `terminated or truncated`. So when the orchestrator forwarded `--episode-steps 35040` the environment built with `episode_time_steps=35040` and RBC honoured it, but IQL and CQL each returned exactly 6000 steps regardless of what the environment reported. CityLearn's normalised KPIs are computed relative to a no-control baseline over the *same* horizon as the policy — `cost_total = 1.0` means "policy produced the same district cost as doing nothing over exactly this many steps" — so comparing a 35,039-step RBC number against a 6000-step IQL number is not a rescaled comparison, it is a different measurement entirely. The fact that the code path silently accepted the mismatch, produced valid-looking JSON, and left no warning in the log made this a much more dangerous bug than a crash would have been.

**Decision matrix.** Three options: (a) rerun the entire benchmark from scratch including RBC, (b) rerun only IQL+CQL and splice the previous run's RBC block back in, (c) keep both horizons and disclose the mixed-horizon caveat. Option (c) was rejected on scientific grounds — the numbers would not be comparable even with disclosure. Option (a) would have cost another ~7 hours of RBC rollout on top of the ~10 hours estimated for IQL+CQL, and `rbc_rollout` was never affected by the bug so the RBC block from the tainted run was still valid on its own terms. Option (b) reproduces the same simulator state the RBC block was measured against (the RNG-seeded env for seeds 200..209) at a fraction of the cost, provided the tooling could splice cleanly. Option (b) was picked.

**The fix.** Three changes to [`scripts/benchmark_entity_agents.py`](../../scripts/benchmark_entity_agents.py), landing together in commit `357997a`:

1. `entity_rollout(..., max_steps: Optional[int] = None)`. The default is now `None` and the terminating branch reads `if max_steps is not None and step >= max_steps: break`. Callers can still pass an explicit `int` for bounded smoke rollouts (used in the test module itself and in the 96-step pre-launch validation), but the runner never sets it, so the environment's `episode_time_steps` drives termination in production.
2. `--skip-rbc`. A CLI flag that entirely suppresses the RBC rollout loop in `main()`. Necessary because rerunning RBC would waste the hours it took the first time; RBC was correct, we just could not reuse it in-place without the next flag.
3. `--merge-existing PATH`. A CLI flag that reads `PATH` as a prior `results.json` and copies its `RBCSmart` block into the freshly-produced output. Overrides any freshly-computed RBC block unconditionally so the semantics stay simple: "with `--merge-existing`, RBC is whatever `PATH` said, IQL and CQL are whatever this run produces". The prior tainted run is preserved on disk as `runs/offline_iql_cql_initiative_15min/benchmark/results.bug9-mixed-horizon.20260705-105300.json` for provenance.

**Validation.** A 96-step smoke test drove one IQL and one CQL rollout for one eval seed against a stub env, with `--skip-rbc --merge-existing` pointing at the tainted results.json, and confirmed the RBC block survived unchanged into the output while the two learned entries carried the expected `steps: 96`. The nine unit tests added alongside the fix (four for the argparse contract, two for `entity_rollout` signature and no-cap behaviour, one for the explicit-cap use case, one asserting `rbc_rollout` is not called under `--skip-rbc`, one asserting `merge-existing` splices the RBC block verbatim) lock the fix at behavioural granularity — see [`tests/scripts/test_benchmark_args.py`](../../tests/scripts/test_benchmark_args.py) and [`tests/scripts/test_benchmark_rollout.py`](../../tests/scripts/test_benchmark_rollout.py). Full pytest post-fix is 794 passed; the subsequent curate-script enhancement (commit `8a54230`, which added Wilcoxon-signed-rank helpers and a `wilcoxon_report.json` side effect to `_render_benchmark_kpi_bars`) brings the count to 800 passed.

The production re-run intended as the final end-to-end validation of the fix was launched (`.venv/bin/python -m scripts.benchmark_entity_agents --episode-steps 35040 --skip-rbc --merge-existing <tainted>`) and ran cleanly for approximately eleven hours before being terminated overnight by an OS-level process suspension at approximately 34% completion (last log line: `IQL seed=206 step 31000`, out of 20 total rollouts at 35,040 steps each). The `benchmark_entity_agents.py` writer only persists to disk at the end after all rollouts complete, so the ~240,000 steps of completed work were not recoverable. This operational failure does not affect the correctness of the Bug 9 fix itself — the fix is validated by the smoke test and the unit test suite — but it did mean that a corrected full-year benchmark table was not produced within thesis timeline. §7 continues to report the 1-day baseline; the operational limitation is disclosed in §10 as future work, and the corrective rerun infrastructure (`--skip-rbc`, `--merge-existing`) remains in the codebase and ready to be re-invoked under `caffeinate -imsu` or an equivalent sleep-blocker.

**Lesson.** Bug 9 is the shortest of the three bugs in code diff (a one-line default and two guard clauses) but the one that would have most damaged the thesis if it had shipped undetected. The tainted `results.json` was internally consistent — no crash, no NaN, all thirty entries populated — and only became obviously wrong under the "does this direction make physical sense" sniff test. Two structural lessons: (a) any function that takes a cap should either take it required or default to unbounded, never both an `int` default *and* a silent `break`, because the two together look like a safety net when they are in fact a lie; and (b) reporting a `steps` field per rollout costs one integer of output and would have surfaced this bug at the first `head results.json` inspection. The step-count field now reflects the environment's terminal step count rather than any local counter, which made post-fix verification a one-line jq query. The operational-failure footnote adds a third lesson that is not about code: **on macOS, any research process expected to run overnight must be launched under `caffeinate` (or the workstation put in the "prevent sleep" energy mode) — the default power-management behaviour will suspend the process, the TTY, and the SSH session together, and none of that will appear in the process log.**
...
>
> _Caveat on rollout length. Each eval episode reported above runs 95 steps (~24 hours of 15-min control), not a full year. The ten env seeds give us ten independent days rather than ten full-year traces, which means the bench has good across-day variance but says nothing about across-season generalisation. Phase 13 attempted a full-year (35,040-step) re-run of the same checkpoints to close this gap: the first attempt was silently truncated to 6,000 steps per algorithm by a bug in the rollout loop (§5.9), and the corrective re-run after the fix was terminated overnight by an OS-level process suspension at ~34% completion. Neither attempt produced usable IQL/CQL numbers within thesis timeline, so the 1-day results above stand as reported. The Bug 9 fix and the corrective rerun infrastructure (`--skip-rbc`, `--merge-existing`) are committed, tested, and ready to re-invoke under `caffeinate` for a follow-up sweep; the full-year evaluation is carried forward as an explicit item in §10._

**Operational-scale reading (illustrative).** The KPIs above are CityLearn normalised ratios, which are useful for cross-policy comparison but do not carry monetary units on their own. To give a sense of scale, consider a plausible 17-building small-to-medium commercial district with an aggregate annual electricity draw on the order of 3-7 GWh (a per-building band consistent with typical European commercial-building energy-use intensity of 150-250 kWh/m² × 1,000-2,500 m² floor area) at a commercial tariff on the order of €0.15-€0.25/kWh. That places the district's uncontrolled baseline annual electricity cost in the €450k-€1.75M range. Applied naïvely, the IQL policy's district-level 46.5% cost reduction translates to €210k-€810k in annual savings for a district of that scale, and CQL's 30.9% to €140k-€540k — both large enough to motivate the applied case for offline RL in building energy management, even acknowledging that (i) the CityLearn-normalised cost ratio does not perfectly commute with real tariff structures (time-of-use, demand charges, PV feed-in), and (ii) these ranges are illustrative and require locality-specific baselining before any deployment claim. The point is directional: at any plausible district scale, a robust cost reduction of the magnitude reported in §7 corresponds to six-figure annual operational savings, which is the range at which offline RL becomes commercially interesting rather than academically interesting.
...
- **1-day vs full-year rollout horizon.** All benchmark KPIs in §7 are computed over 95-step (approximately 1-day) evaluation rollouts across ten disjoint env seeds. Cross-day variance is well-characterised (n=10 with paired-Wilcoxon), but cross-season generalisation is not tested. Two Phase 13 attempts at full-year (35,040-step) rollouts did not complete within thesis timeline: the first was invalidated by Bug 9 (§5.9), and the corrective retry after the fix was terminated overnight by macOS process suspension at ~34% completion. The corrected rollout pipeline is in the repository and would take approximately 30-40 h of wall-clock to run to completion under `caffeinate -imsu`; this is the natural first follow-up before any deployment claim. The 1-day results reported in §7 remain internally valid at the horizon they disclose, but they should not be extrapolated to full-year operating regimes without this follow-up sweep.
- **Cross-year weather generalisation is untested.** All eval seeds sample from the same synthetic weather distribution as the collection seeds. Even a successful full-year rollout (per the item above) would only exercise one weather year. A production-oriented follow-up should evaluate the same checkpoints against multiple real weather years spanning the range of the deployment target's climate variability, with the reward-function coefficients potentially re-tuned per climate regime.
