...
> The dataset is a flat table of `(obs, action, reward, next_obs)`
...
├── derived/
│ └── rbc_with_reward.parquet # one row per transition, all seeds, with reward column
...
target — verify variance!), **2 reward columns** (env + ).
...
> zeros) is impossible by construction in because there is no
...
| `reward` | float | The **KPI-aligned reward** computed by `algorithms/offline_rl/reward.py`. **This is the training signal for BC/IQL.** See `reward_design.md`. |
...
action column — the M2 failure mode is a hard error in .
