...
> others. v2 fixes that with a **KPI-aligned weighted sum** whose
...
must mention ramping. v1 didn't; v2 does.
...
> `reward_v2_t = − ( w_cost · C_t + w_carbon · G_t + w_peak · P_t + w_ramp · R_t + w_unserved · U_t )`
...
v2 keeps the env reward in the dataset as `reward_env` for traceability
but trains on `reward_v2`, which is **(i) per-building, (ii) aligned
...
1. Run RBC for `K = 10` seeds (the v2 collector already produces
...
6. Sanity-check: the resulting `reward_v2` summed across each rollout
...
calibration above and are recorded in `reward_v2_weights.json`.
...
`algorithms/offline_rl_v2/reward_v2.py` exports
...
def compute_reward_v2(
    ...
    *,
    weights: dict,       # loaded from reward_v2_weights.json
...
(`reward_v2_breakdown.parquet`) for analysis. We don't add the
breakdown columns to the main dataset to keep the schema lean.

### 5.2 Unit tests (`tests/offline_rl_v2/test_reward_v2.py`)
...
| `test_reward_v2_outperforms_v1_on_peaks` | On a synthetic "peak day", v2 penalises the peak hour and v1's reward does not. |

### 5.3 Calibration script

`scripts/calibrate_reward_v2.py`:

- Reads `datasets/offline_rl_v2/rbc/seed_*.parquet`.
- Computes per-step terms with `compute_reward_v2(..., weights=ones)`.
- Computes per-rollout KPIs by replaying the env (or by reading
  `kpi_summary.csv` if already cached).
- Fits weights as in §4.1.
- Writes `datasets/offline_rl_v2/derived/reward_v2_weights.json`.
...
|----------|-------------|
| Frozen weights | `datasets/offline_rl_v2/derived/reward_v2_weights.json` |
| Per-step `reward_v2` values | `derived/rbc_with_reward_v2.parquet` (column) |
| Per-step term breakdown | `derived/reward_v2_breakdown.parquet` |
| Calibration log | `derived/reward_v2_calibration.log` (procedure, regression diagnostics, final weights, sanity-check ρ) |

When a future iteration swaps the behaviour policy (e.g. RBC → BC-v2),
...
3. **Reward leaks future info.** `compute_reward_v2` only consumes
