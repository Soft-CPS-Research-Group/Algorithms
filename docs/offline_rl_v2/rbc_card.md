# RBC Card — Behaviour Policy v2

> The Rule-Based Controller (RBC) is the **behaviour policy** that
> generates the v2 dataset. This card describes *what it does*, *why
...
- **Dataset diversity.** The RBC's behaviour is mostly deterministic
  ...
  → If IQL-v2 plateaus at RBC level, the next iteration must use a
  noisier or learned behaviour policy to broaden action coverage.
- **Reward design.** The RBC's blind spots (peak, ramping, price,
  carbon, idle battery) tell us exactly which terms the v2 reward must
  include if we want the agent to do better than imitate. See
  `reward_design_v2.md`.
- **Evaluation.** Any v2 agent that ties RBC on EV-deadline metrics but
