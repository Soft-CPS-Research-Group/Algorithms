...
def test_signal_aware_strict_local_linear_discount_has_continuous_authority():
    agent = SignalAwareRBCSmartLocal(
        {
            "algorithm": {
                "hyperparameters": {
                    ...
                    "signal_price_charge_reference_multiplier": 0.9,
    ...
    assert agent.predict(observations, context=1.0) == [[0.0]]
    assert agent.predict(observations, context=0.95) == [[pytest.approx(0.3)]]
    assert agent.predict(observations, context=0.90) == [[pytest.approx(0.6)]]
