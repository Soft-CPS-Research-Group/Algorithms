"""Unit tests for ``algorithms.offline_rl_v2.reward_v2``.

Implements the test list from ``docs/offline_rl_v2/reward_design_v2.md`` §5.2.
...
from algorithms.offline_rl_v2 import reward_v2 as R
...
def test_finite_on_zero_action():
    ...
    nobs = make_next_obs()
    r, terms = R.compute_reward_v2(obs, np.zeros(2), nobs, weights=R.DEFAULT_WEIGHTS, state=state)
...
def test_sign_cost_negative():
    ...
    nobs = make_next_obs()
    r, terms = R.compute_reward_v2(obs, None, nobs, weights={"cost": 1.0, "carbon": 0, "peak": 0, "ramp": 0, "unserved": 0}, state=state)
...
def test_vectorised_matches_loop():
    ...
    df = pd.read_parquet("datasets/offline_rl_v2/rbc/seed_22.parquet")
...
def test_reward_v2_outperforms_v1_on_peaks():
    ...
    for _ in range(R.PEAK_WINDOW_HOURS):
        R.compute_terms(base_obs, make_next_obs(), state=state_v2)

    # v1-style "reward" = -net (cost only, equal price/carbon weight).
    v1_base_reward = -1.0
    v1_spike_reward = -10.0

    # v2: same scenario, full reward.
    base_r, _ = R.compute_reward_v2(base_obs, None, make_next_obs(), weights=R.DEFAULT_WEIGHTS, state=R.RewardState())
    # warm v2 then spike
    state_v2 = R.RewardState()
    for _ in range(R.PEAK_WINDOW_HOURS):
        R.compute_terms(base_obs, make_next_obs(), state=state_v2)
    spike_r, spike_terms = R.compute_reward_v2(
        spike_obs, None, make_next_obs(), weights=R.DEFAULT_WEIGHTS, state=state_v2
    )

    # The spike must be punished more in absolute terms by v2 than the
    # ratio implied by v1 (-10 vs -1 = factor 10).
    v1_ratio = v1_spike_reward / v1_base_reward  # = 10
    v2_ratio = spike_r / base_r if base_r != 0 else float("inf")
    assert v2_ratio > v1_ratio, (
        f"v2 should penalise peaks more than v1 ratio. "
        f"v1_ratio={v1_ratio}, v2_ratio={v2_ratio}, spike_terms={spike_terms}"
    )
