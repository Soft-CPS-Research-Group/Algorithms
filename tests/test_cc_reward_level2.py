...
# ---------------------------------------------------------------------------
# Per-building credit mode (reward_mode="per_building")
# ---------------------------------------------------------------------------

def test_invalid_reward_mode_raises():
    with pytest.raises(ValueError):
        make_l2(reward_mode="nonsense")


def test_per_building_preserves_community_total():
    """The zero-mean spatial term + shared community scalar leave the *total*
    reward (Σ over buildings) unchanged vs aggregate mode when there are no EVs.

    This is the key property: per-building mode only *redistributes* the reward
    to create a differentiation gradient; it does not move the overall objective.
    """
    observations = [
        obs(5.0, price=0.30, violation=0.5),
        obs(-3.0, price=0.30),
        obs(2.0, price=0.30),
    ]
    agg = make_l2(reward_mode="aggregate")
    pb  = make_l2(reward_mode="per_building", w_spatial=1.0)
    assert sum(pb.calculate(observations)) == pytest.approx(sum(agg.calculate(observations)))


def test_per_building_differentiates_by_import():
    """Buildings importing more than average get a lower (more negative) reward;
    net exporters get a higher one — the spatial gradient that was missing."""
    reward = make_l2(
        reward_mode="per_building", w_spatial=1.0,
        w_cost=0.0, w_peak=0.0, w_ramp=0.0, w_export=0.0, w_violation=0.0,  # isolate spatial
    )
    # b0 heavy import, b1 light import, b2 net export — all same price.
    result = reward.calculate([obs(8.0, price=0.30), obs(1.0, price=0.30), obs(-4.0, price=0.30)])
    assert result[0] < result[1] < result[2]           # more import ⇒ lower reward
    assert result != pytest.approx([result[0]] * 3)     # genuinely differentiated


def test_per_building_reduces_to_aggregate_when_flat():
    """If every building has the identical net/price and no EVs, the zero-mean
    spatial term vanishes and per-building == aggregate elementwise."""
    observations = [obs(2.0, price=0.2), obs(2.0, price=0.2), obs(2.0, price=0.2)]
    agg = make_l2(reward_mode="aggregate")
    pb  = make_l2(reward_mode="per_building", w_spatial=1.0)
    assert pb.calculate(observations) == pytest.approx(agg.calculate(observations))


def test_per_building_attributes_ev_harm_to_the_right_building():
    """The EV penalty lands on the building whose EV is at risk, not spread evenly."""
    reward = make_l2(
        reward_mode="per_building", w_spatial=0.0,   # isolate the EV term
        w_cost=0.0, w_peak=0.0, w_ramp=0.0, w_export=0.0, w_violation=0.0,
        w_ev=0.5, urgency_horizon=4.0,
    )
    result = reward.calculate([
        obs(0.0, evs={"c1": ev(hours_until_departure=1.0)}),   # harm 0.30
        obs(0.0),                                              # no EV
    ])
    # Building 0 carries the full -w_ev*harm; building 1 carries none.
    assert result[0] == pytest.approx(-0.5 * 0.30)
