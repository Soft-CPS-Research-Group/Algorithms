"""Tests for BC (policy, dataset, trainer, agent)."""
...
from algorithms.offline_rl.bc_agent import BCAgent
...
from algorithms.offline_rl.bc_trainer import (
    BCTrainingConfig,
    train_single_seed,
)
...
def test_dataset_action_targets_finite_and_in_range() -> None:
    ...
    # Sanity: the storage-action column is constant 0 in RBC.
...
def test_policy_forward_shape() -> None:
    policy = BCPolicy(obs_dim=35, action_dim=2, hidden_layers=[32, 32], dropout=0.0)
...
def test_training_overfits_tiny_subset(tmp_path: Path) -> None:
    ...
    # Build a parquet that satisfies load_obs_action_arrays: requires the
...
# ---------------------------------------------------------------------------
# 6. BCAgent.predict returns 17 action vectors after attach_environment;
...
def test_agent_predict_returns_17_action_vectors(tmp_path: Path) -> None:
    ...
    agent = BCAgent.from_seed_dir(out)
    ...
    for i in range(len(actions)):
        ...
        assert actions[i] == rbc_actions[i], (
            f"BCAgent changed off-target building {i}'s action; should defer to RBC"
        )
...
def test_agent_obs_standardisation_changes_outputs() -> None:
    ...
    policy = BCPolicy(obs_dim=obs_dim, action_dim=action_dim, hidden_layers=[16, 16], dropout=0.0)
    ...
    agent = BCAgent(policy, standardiser)

    # The agent's RBC is a real OfflineRBC, but we want to isolate
...
def test_train_persists_best_epoch_weights(tmp_path: Path) -> None:
    ...
    from algorithms.offline_rl.bc_dataset import load_and_split
