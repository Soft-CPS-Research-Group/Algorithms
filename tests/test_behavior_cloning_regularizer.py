...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.full(6, np.nan), [0.25], "observation_nonfinite", "(6,)", "(6,)", 1.0),
        (np.zeros(6), [0.25, 0.5], "target_shape_mismatch", "(1,)", "(2,)", 1.0),
        (np.zeros(6), [float("nan")], "target_nonfinite", "(1,)", "(1,)", 1.0),
    ],
)
...
def test_state_round_trip_restores_read_only_demonstration_arrays() -> None:
    regularizer = _regularizer()
    layout = _layout()
...
def test_demonstration_loss_normalizes_nonzero_weight_sums_below_one() -> None:
    regularizer = _regularizer(ev_multiplier=0.5, weight=1.0)
    layout = _layout()
    regularizer.record_demonstration(0, np.zeros(3), layout, [0.0])

    loss = regularizer.demonstration_loss(
        layout=layout,
        demonstrations=regularizer.sample_demonstrations(0, layout, batch_size=1),
        predicted_means=torch.tensor([[[2.0]]]),
        global_learning_step=0,
    )
...
def test_compute_teacher_actions_is_deterministic_and_returns_a_copy() -> None:
    class Teacher:
    ...
    actions = regularizer.compute_teacher_actions([np.zeros(3)])
    teacher.actions[0][0] = 0.75

    assert teacher.calls[0][1] is True
    assert actions == [[0.25]]


def test_attach_environment_builds_and_retains_teacher(monkeypatch) -> None:
    regularizer = _regularizer()
    teacher = object()
    captured = {}

    def build_teacher(observation_names, action_names, action_space, observation_space, metadata):
        captured.update(
    ...
    monkeypatch.setattr(regularizer, "_build_teacher_policy", build_teacher)
    regularizer.attach_environment(
        observation_names=[["observation"]],
        action_names=[["action"]],
        action_space=["action-space"],
        observation_space=["observation-space"],
        metadata={"topology_version": 1},
    )

    assert regularizer.teacher_policy is teacher
    assert captured["metadata"] == {"topology_version": 1}


def test_unsupported_teacher_policy_fails_before_construction() -> None:
    regularizer = _regularizer(teacher={"policy": "UnsupportedPolicy"})

    with pytest.raises(ValueError, match="Unsupported AgentTransformerPPO BC teacher"):
        regularizer._build_teacher_policy([], [], [], [], None)
