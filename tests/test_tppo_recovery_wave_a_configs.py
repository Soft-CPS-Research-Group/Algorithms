...
def test_bc_tppo_configs_have_demo_ppo_eval_phases(configs: dict[str, dict]) -> None:
    ...
    assert auxiliary_bc["decay_start_step"] == ppo_start_step
    assert auxiliary_bc["decay_steps"] == ppo_end_step - ppo_start_step
    ...
    assert regularizer.effective_weight(ppo_start_step) == pytest.approx(auxiliary_bc["weight"])
